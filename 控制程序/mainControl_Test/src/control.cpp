...
// 全局雷达参数实例
RADAR_PARAMS radar_params = {0, false, 0, false, false, 480, true}; // 初始化全局结构体变量，默认开启雷达
...
void *udp_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        // 重置状态跟踪
        port8000_received = false;
        port8002_received = false;
        ...
        // printf("Select activity: %d, checking sockets...\n", activity);
        ...
        if (FD_ISSET(sockfd_insert, &readfds))
        {
            ...
            if (ret > 0)
            {
                ...
                /*
                // 打印mapData的信息
                printf("Raw buffer content (first 20 bytes): ");
                for(int i = 0; i < 20; i++) {
                    printf("%02X ", (unsigned char)buffer_insert[i]);
                }
                printf("\n");
                
                printf("mapData fields after memcpy:\n");
                printf("version: %d\n", mapData.version);
                printf("frameNum: %d\n", mapData.frameNum);
                printf("frameId: %d\n", mapData.frameId);
                printf("validNumInFrame: %d\n", mapData.validNumInFrame);
                
                printf("MapToControl structure info:\n");
                printf("sizeof(MapToControl): %zu\n", sizeof(MapToControl));
                printf("offsetof(validNumInFrame): %zu\n", offsetof(MapToControl, validNumInFrame));
                
                if (mapData.validNumInFrame == 0) {
                    printf("mapData is empty after memcpy (validNumInFrame = 0)\n");
                } else {
                    printf("mapData.validNumInFrame: %d\n", mapData.validNumInFrame);
                }
                */
