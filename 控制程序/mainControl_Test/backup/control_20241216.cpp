...
#define SERIAL_PORT "/dev/ttyS0" 	// 串口设备路径
...
void handle_sigalrm(int sig) {
	...
	memcpy(frame_data, frame.data, 8);
...
void readMapDataFromFile(const string& filePath) {
    ...
    while (getline(file, line)) {
        ...
        if (lat_pos != string::npos && lon_pos != string::npos) {
            try {
                ...
                if (lat_end == string::npos) lat_end = line.find(" ", lat_start);
                if (lon_end == string::npos) lon_end = line.find(" ", lon_start);
    ...
    file.close();
    cout << "Successfully read " << map_latitude_v.size() << " map points." << endl;
}
...
bool parseGPCHC(const std::string& sentence, double& latitude, double& longitude, double& heading, int& status) {
    ...
    try {
        heading = std::stod(fields[14]);    // 偏航角
...
std::pair<double, double> readSerialData(int fd) {
    char buffer[256];
    std::string sentence;
    double latitude = 0.0, longitude = 0.0, heading = 0.0;
    int status = 0;
	std::pair<double,double> res(11,22);
    ...
    while (true) {
        ...
        if (bytesRead < 0) {
            ...
            if (pos != std::string::npos) {
                std::string fullSentence = sentence.substr(0, pos);
                sentence.erase(0, pos + 1);  // 正确删除换行符

                if (parseGPCHC(fullSentence, cur_params.latitude, cur_params.longitude, cur_params.heading_angle, status)) {
                    // 实时更新经纬度变量并打印
                    std::cout << "Updated Latitude: " << cur_params.latitude
                              << ", Longitude: " << cur_params.longitude
                              << ", Heading: " << cur_params.heading_angle
                              << ", Status: " << status << std::endl;
                    res.first=cur_params.latitude;
                    res.second=cur_params.longitude;
                    return res;
                } else {
   ...
   /* string newFileName(int kid)
	...
	void createFile(string output_path)
    {
        ...
        if(fp)
        {
            ...
            record.append(",");
            record.append("heading_angle");    			// 本車航向角
            record.append(",");            
 	...
 	void saveRecord(string control_log_path, vector<double> data_vd, vector<double> dis_vd) 
    {
    	time_t t = time(0);
        char tmp[64];
        if (fp)
        {
            ...
            strftime(tmp, sizeof(tmp), "%Y%m%d%H%M%S",localtime(&t) ); 
            string record = "";  
            record.append(tmp); 									//时间戳
            record.append(",");
            record.append(tmp);    									// 知识时间戳
            record.append(",");
            
            //gps组
            record.append(doubleToString(cur_params.latitude));    				// 本車纬度
            record.append(",");
            record.append(doubleToString(cur_params.longitude));    				// 本車经度
            record.append(",");
            record.append("heading_angle");    			// 本車航向角
            record.append(",");            
            record.append("map_latitude"); 				// 目標點纬度
...
void initLogFile() {
    ...
    std::string logFilePath = "/home/ztl/log/log_" + timestamp + ".txt";
...
// 保存日志到文件
void logToFile(double vehicle_speed, double latitude, double longitude, double heading_angle,
               double map_latitude, double map_longitude, double Ld, double alpha, uint8_t* frame_data) {
    // 获取当前时间戳
    std::string timestamp = getCurrentTimestamp();

    // 日志文件路径和名称（精确到秒）
    std::string logFilePath = "/home/ztl/log/log_" + getCurrentTimestamp().substr(0, 19) + ".txt";
    ...
    logFile << timestamp << ", "
            ...
            << "CAN Frame: 0x410, Data: ";

    // 记录CAN报文的8字节数据
    for (int i = 0; i < 8; ++i) {
        logFile << std::setw(2) << std::setfill('0') << std::hex << (int)frame_data[i];
        if (i < 7) logFile << " ";
    }
...
void* main_thread(void* arg) {
    ...
    while(1) {
		...
		double s12(0), azi1(0), azi2(0);
		double steer_error;
		std::pair<double, double> real_gps;
		//while(1)
		//{
			real_gps = readSerialData(serialFd);
			printf("\nrev_data[1] = %d\n",rev_data[1]);
			
			cur_params.latitude = real_gps.first;
			cur_params.longitude = real_gps.second;
			cur_params.vehicle_speed = (double)rev_data[1];
		
			printf("\ncur_params.latitude = %f\ncur_params.longitude = %f\ncur_params.heading_angle = %f\n",cur_params.latitude,cur_params.longitude,cur_params.heading_angle);
			printf("\ncur_params.vehicle_speed = %f\n",cur_params.vehicle_speed);
		//}
		pthread_mutex_lock(&mutex);
		...
		//cur_params.latitude = 39.20842796;
		//cur_params.longitude = 117.22737372;
		//cur_params.heading_angle = 160.0;
 
 		steer_error = GPS2Steer(cur_params.vehicle_speed, cur_params.latitude, cur_params.longitude, cur_params.heading_angle, map.latitude, map.longitude, wheelbase, Ld, alpha);
		...
		printf("\n\nStart Can \n\n");
        can_vcu_in.gear = 1;
        can_vcu_in.endSpeed = 2;
        
        can_vcu_in.keepAngle = steer_error;
      ...
      /*  if(add_sig ==1)
        ...
        else
        {
 		...
 		logToFile(vehicle_speed, cur_params.latitude, cur_params.longitude, cur_params.heading_angle, map.latitude, map.longitude, Ld, alpha, frame_data);	
        // 休眠20毫秒
        usleep(20000);
