...
void readSerialData(int fd, const std::string& customFileName = "")
{
    char buffer[256];
    ...
    if (!customFileName.empty()) {
        ...
        filePath = directory + getCurrentTimeString();
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        std::cout << "Using custom filename: " << customFileName << std::endl;
    } else {
        std::cout << "No custom filename provided, using timestamp." << std::endl;
    }
    ...
    readSerialData(serialFd, customFileName);
