...
#define SERIAL_PORT "/dev/ttyS7"  // 串口设备路径，修改为实际的串口设备路径
#define BAUD_RATE B460800          // 波特率设置为 460800
...
// 解析十六进制数据，提取经纬度、航向角、状态等信息
std::vector<double> processHexData(std::vector<uint8_t>& data) {
    double posLat = 0.0, posLog = 0.0;
    double insFlag = 0.0;
    double headingAngle = 0.0, vehicleSpeed = 0.0;
    // 创建一个包含5个元素的vector，依次存储：latitude, longitude, heading_angle, insFlag, vehicleSpeed
    std::vector<double> res(5, 0.0);
    
    // 这里要求data.size()应为93（对应原始hex字符串长度186的转换结果）
    if (data.size() == 93) {
        // 解析 insFlag（从下标59处取1个字节，直接转为double）
        insFlag = static_cast<double>(data[59]);
        
        // 解析纬度 (从下标21开始取8个字节，LSB first，即小端字节序)
        if (data.size() > 21 + 7) {
            std::memcpy(&posLat, &data[21], sizeof(double));
        }
        
        // 解析经度 (从下标29开始取8个字节，LSB first)
        if (data.size() > 29 + 7) {
            std::memcpy(&posLog, &data[29], sizeof(double));
        }
        
        // 解析航向角 (从下标9开始取4个字节，LSB first)
        if (data.size() > 9 + 3) {
            float headingTemp = 0.0f;
            std::memcpy(&headingTemp, &data[9], sizeof(float));
            headingAngle = static_cast<double>(headingTemp);
        }
        
        // 解析车速 (从下标61开始取4个字节，LSB first)
        if (data.size() > 61 + 3) {
            float speedTemp = 0.0f;
            std::memcpy(&speedTemp, &data[61], sizeof(float));
            vehicleSpeed = static_cast<double>(speedTemp);
        }
        ...
        std::cout << "Parsed Info: "
                  << "Latitude: " << std::fixed << std::setprecision(8) << posLat
                  << ", Longitude: " << std::fixed << std::setprecision(8) << posLog
                  << ", Heading: " << std::fixed << std::setprecision(8) << headingAngle
                  << ", insFlag: " << insFlag
                  << ", VehicleSpeed: " << vehicleSpeed << std::endl;
    } 
    else
    {
        std::cerr << "Error GPS message." << std::endl;
        std::cout << "ERROR data size =" << data.size() << std::endl;
    }
